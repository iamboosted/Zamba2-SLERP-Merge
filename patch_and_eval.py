...
# Smarter patch: skip only the validation, not the tying itself
original_fn = transformers.modeling_utils.PreTrainedModel.get_expanded_tied_weights_keys
def patched_fn(self, all_submodels=False):
    try:
        return original_fn(self, all_submodels=all_submodels)
    except ValueError:
        print("  [PATCH] Skipping tie_weights validation")
        return {}
transformers.modeling_utils.PreTrainedModel.get_expanded_tied_weights_keys = patched_fn
...
MODELS = {
    "zamba2_base":     "Zyphra/Zamba2-7B",
    "zamba2_instruct": "Zyphra/Zamba2-7B-Instruct",
    "zamba2_merged":   "/workspace/zamba2-7b-merged",
}
...
for model_name, model_path in MODELS.items():
    ...
    results[model_name] = {"score": score, "correct": correct, "total": total, "details": details}
...
for name, r in results.items():
    print(f"  {name:20s}: {r['correct']}/{r['total']} ({r['score']:.0f}%)")

with open("/workspace/zamba_eval_results.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"\nResults saved to /workspace/zamba_eval_results.json")
